...
import { distancematrix } from '@/api/mapsDistance'
...
const MapComponent = ({
  ...
  const memorisedMarkers = useMemo(
    () => markers?.filter((data) => showDataStatus[data?.locationType]),
    [markers, showDataStatus],
  )
  ...
  const { mutate: getDistanceMatrix, isLoading: tableDataLoader } = useMutation(
    ({ destination, origin }: { destination: string; origin: string }) =>
      distancematrix(destination, origin),
    {
      onSuccess: (data) => {
        console.log('===== distance', data)

        let distance = data.data.rows[0].elements[0].distance,
          duration = data.data.rows[0].elements[0].duration_in_traffic,
          duration_traffic = data.data.rows[0].elements[0].duration,
          velocity = Math.round((distance / duration_traffic) * 60),
          avg_delay = Math.round(duration_traffic / distance)
        setGraphData(data)
      },
      onError: (error) => {
  ...
  useEffect(() => {
    getDistanceMatrix({ destination: '25.673569, -100.367882', origin: '25.658122, -100.370288' })
  }, [])
...
<circle cx="${markerdData.empno / 8}" cy="${markerdData.empno / 8}" r="${
              markerdData.empno / 8
            }" fill="red"/>
</svg> `
...
<circle cx="${markerdData.empno / 8}" cy="${markerdData.empno / 8}" r="${
          ...
          {infoLocation ? (
            <InfoWindow position={infoLocation} onCloseClick={clearInfoWindow}>
              <InfoBoxComponent
                title={infoLocationData?.MEEDTitle}
